...
const AUTH_BACKGROUNDS = ['gorra2.jpg', 'gorra3.jpg', 'gorra4.jpg'];

function AuthBackdrop({ title, text }) {
  return (
    <div className="hidden lg:block relative bg-charcoal-950 overflow-hidden">
      ...
      <div className="relative h-full flex flex-col justify-center p-12 -translate-y-8 auth-copy-lift">
        ...
        <h2 className="text-4xl font-black text-white tracking-tight leading-tight mb-3">{title}</h2>
        <p className="text-zinc-300 leading-relaxed max-w-sm">{text}</p>
      </div>
    </div>
  );
}
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (status === 429) {
        ...
        setErrors({ general: msg || 'Tu cuenta esta suspendida' });
      } else {
        setErrors({ general: msg || 'Correo o contrasena incorrectos' });
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-[100dvh] grid grid-cols-1 lg:grid-cols-2">
      <div className="flex items-center justify-center px-6 py-16 bg-cream dark:bg-charcoal-950 overflow-y-auto">
        <div className="w-full max-w-sm">
          <div className="mb-8 animate-fade-up">
            <p className="text-gold dark:text-gold-light text-xs font-semibold uppercase tracking-widest mb-2">Bienvenido</p>
            <h1 className="text-3xl font-black text-charcoal-950 dark:text-zinc-50 tracking-tight">Iniciar sesion</h1>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4 animate-fade-up delay-100">
            ...
            <Input id="email" name="email" type="email" label="Correo electronico"
      ...
      <AuthBackdrop
        title={<>Tu estilo,<br />siempre contigo.</>}
        text="Accede a tu cuenta para gestionar pedidos, guardar direcciones y comprar tus gorras favoritas."
      />
    </div>
  );
}
